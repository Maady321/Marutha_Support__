// volunteer_details.js - Volunteer Directory (Doctor View)
// Shows all volunteers with their stats and reports
...
function populateProfile() {
    ...
    .then(function(data) {
        ...
        var initEl = document.getElementById("docInitial");
...
async function loadVolunteerDirectory() {
    var tableBody = document.getElementById('volunteer-details-table-body');
    if (!tableBody) return;

    try {
        var token = localStorage.getItem('auth_token');
        var res = await fetch(CONFIG.API_BASE_URL + '/doctors/volunteers-full-details', {
            headers: { 'Authorization': 'Bearer ' + token }
        });

        if (!res.ok) throw new Error('Failed to load');

        var volunteers = await res.json();
        ...
        for (var i = 0; i < volunteers.length; i++) {
            var vol = volunteers[i];
            var tr = document.createElement('tr');

            // Format time
            var totalSec = Math.floor(vol.total_hours * 3600);
            var hrs = Math.floor(totalSec / 3600);
            var mins = Math.floor((totalSec % 3600) / 60);
            var secs = totalSec % 60;
            var formattedTime = '<span style="font-weight: 700">' + hrs + 'h ' + mins + 'm ' + secs + 's</span>';

            // Get initials (first 2 chars)
            var initials = vol.name.substring(0, 2).toUpperCase();

            // Reports button
            var reportsBtn = '';
            if (vol.recent_reports.length > 0) {
                reportsBtn = '<button class="btn btn-primary btn-sm" onclick="viewReports(' + vol.id + ')" title="View Reports"><i class="fas fa-file-alt"></i></button>';
            }

            tr.innerHTML =
                '<td>' +
                    '<div style="display:flex; align-items:center; gap:12px;">' +
                        '<div style="width: 40px; height: 40px; border-radius: 50%; background: var(--lavender-soft); color: var(--medical-blue); font-weight:700; display:flex; align-items:center; justify-content:center;">' + initials + '</div>' +
                ...
                '<td>' +
                    '<span style="background: var(--lavender-soft); color: var(--medical-blue); padding: 4px 10px; border-radius: 20px; font-weight: 700; font-size: 0.8rem;">' +
                        vol.patient_count + ' / 3 Assigned</span>' +
                '</td>' +
                '<td>' +
                    '<span style="display:inline-block; margin-right: 6px;"><i class="fas fa-check-circle" style="color:var(--hope-green)"></i></span>' +
                    '<strong style="color:var(--text-main)">' + vol.tasks_completed + '</strong> ' +
                    '<span style="font-size:0.85rem; color:var(--text-muted)">/ ' + vol.tasks_total + '</span>' +
                '</td>' +
                '<td style="color:var(--medical-blue)">' + formattedTime + '</td>' +
                '<td><div style="font-weight: 600; font-size: 1.1rem; color:var(--text-main)">' + vol.reports_total + '</div></td>' +
                '<td>' +
                    '<button class="btn btn-outline-lavender btn-sm" onclick="chatWithVolunteer(' + vol.user_id + ')" title="Message"><i class="fas fa-comment"></i></button> ' +
                    reportsBtn +
                '</td>';

            tableBody.appendChild(tr);

            // Generate reports modal if reports exist
            if (vol.recent_reports.length > 0) {
...
function generateReportsModal(vol) {
    ...
    var htmlContent = '';
    for (var i = 0; i < vol.recent_reports.length; i++) {
        var r = vol.recent_reports[i];
...
function viewReports(volId) {
    var el = document.getElementById('modal-reports-' + volId);
