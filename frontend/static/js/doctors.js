// doctors.js - Load and display list of doctors
// When patient clicks "View Profile", goes to doctor_profile.html?id=X

document.addEventListener('DOMContentLoaded', async function() {
    var doctorsContainer = document.getElementById('doctors-container');
    if (!doctorsContainer) return;

    try {
        // Fetch all doctors from backend
        var response = await apiFetch('/doctors/');
        if (!response.ok) {
        ...
        for (var i = 0; i < doctors.length; i++) {
            ...
            // Get initials
            var nameParts = (doctor.name || 'Dr').split(' ');
            var initials = '';
            ...
            var isAvailable = doctor.is_online;
            ...
            var expText = 'Experience not listed';
            if (doctor.experience) {
                expText = doctor.experience + ' years experience';
            }

            var qualBadge = '<div style="margin-bottom: 20px;"></div>';
            ...
            var statusBg = isAvailable ? '#ecfdf5' : '#fef3c7';
            var statusColor = isAvailable ? '#065f46' : '#92400e';
            var statusBorder = isAvailable ? '#a7f3d0' : '#fcd34d';
            var statusText = isAvailable ? 'Available Now' : 'Offline';

            card.innerHTML =
                '<div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 24px;">' +
                    '<div class="profile-pic" style="width: 80px; height: 80px; font-size: 2rem; background: ' + color.bg + '; color: ' + color.text + '; border: 1px solid rgba(0,0,0,0.05);">' + initials + '</div>' +
                    '<span class="badge" style="background: ' + statusBg + '; color: ' + statusColor + '; border: 1px solid ' + statusBorder + '">' + statusText + '</span>' +
                '</div>' +
                ...
                '<div style="margin-top: auto; display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">' +
                    '<a href="doctor_profile.html?id=' + doctor.id + '" class="btn btn-outline-lavender" style="justify-content: center;">' +
                        '<i class="fas fa-user" style="margin-right: 6px"></i> Profile</a>' +
                    '<a href="doctor_profile.html?id=' + doctor.id + '" class="btn btn-primary" style="justify-content: center;">' +
                        '<i class="fas fa-calendar-plus" style="margin-right: 6px"></i> Consult</a>' +
                '</div>';
